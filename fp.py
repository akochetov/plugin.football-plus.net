...
def openMatch(url):
    ...
    print(match_data)
    pics = re.compile('<meta itemprop="image" content="([^"]+?)"').findall(html)
    match_data=re.compile('"file":"(http://[^"]+?)"').findall(match_data[0])
    
    hits = []    
    for i,dt in enumerate(match_data):
        hits.append(SearchHit(url=match_data[i],title = str(i+1)+u' тайм',image=pics[0]))    
